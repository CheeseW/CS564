...
const void BTreeIndex::insertEntry(const void *key, const RecordId rid) 
{
	...
	insertEntryUpdate(intKey, rid, rootPageNum, true);
}
...
PageKeyPair<int> BTreeIndex::leafNodeInsertSplit(LeafNodeInt* curNode, int key, RecordId rid)
{	
	PageKeyPair<int> feedbackTuple;
	std::cout << "key = " << key << " inserted" << std::endl;
...
void BTreeIndex::nonLeafNodeInsert(NonLeafNodeInt* node, PageKeyPair<int> updateTuple)
{
...
PageKeyPair<int> BTreeIndex::nonLeafNodeInsertSplit(NonLeafNodeInt* curNode, PageKeyPair<int> updateTuple)
{
	PageKeyPair<int> feedbackTuple;
	// Page* curPage;
	// bufMgr->readPage(file, pid, curPage);
	...
	{
		// if the first element in the ori non-leaf node is already larger than the new key
		assert(pos == 0);
		for(int i = 1; i < INTARRAYNONLEAFSIZE + 1; ++i)
		{
			assert(curNode->validArray[i - 1]);
			keyBuffer[i] = curNode->validArray[i - 1];
			pidBuffer[i + 1] = curNode->pageNoArray[i];
		}
	}
...
recurStruct BTreeIndex::insertEntryUpdate(int key, RecordId rid, PageId pid, bool isRoot)
{
	recurStruct returnStruct;
	returnStruct.needsUpate = false;
	returnStruct.updateTuple.set(0, 0);
	// the first call
	...
	if(!hasNonLeaf)
	{
		...
		else
		{
			...
			if(vldCnt == INTARRAYLEAFSIZE)
			{
				...
				tuple = leafNodeInsertSplit(node, key, rid);
	...
	if(pos == 0)
	{
		// if get the first entry, then it must be valid (page non-empty) and its key must be larger than the inserted key
		// if(node->validArray[pos] == false)
		// {
		// 	if(pid == rootPageNum)
		// 		std::cout << "at root page" << std::endl;
		// }
		if(node->validArray[pos] == false)
			std::cout << "faulting node page id = " << pid << std::endl;
		// std::cout << "pos = " << pos << std::endl;
		// std::cout << "key = " << key << std::endl;
		// std::cout << "keyArray[pos] = " << node->keyArray[pos] << std::endl;
		assert(node->validArray[pos] == true);
		assert(node->keyArray[pos] > key);
	}

	if(node->validArray[pos] == false || pos > INTARRAYNONLEAFSIZE)
	{
	...
	if(node->level == 1)
	{
		...
		// for(i = 0; i < INTARRAYNONLEAFSIZE; ++i)
		// {
		// 	if(node->validArray[i] && node->keyArray[i] > key)
		// 		break;
		// }
		// assert(i < INTARRAYNONLEAFSIZE);
		...
		else
		{
			...
			updateTuple = leafNodeInsertSplit(leafNode, key, rid);

			// check if this non-leaf node is full
			if(sum == INTARRAYNONLEAFSIZE)
			{
				// the non-leaf node is full, needs to update upper level
				returnStruct.needsUpate = true;
				returnStruct.updateTuple = nonLeafNodeInsertSplit(node, updateTuple);
	...
	else
	{
		...
		recurStruct updateStruct = insertEntryUpdate(key, rid, nextNonLeafPid, false);
		// if(pid == rootPageNum)
		// 	std::cout << "back at root, also ";
		// std::cout << "back at level = 0" << std::endl;

		if(updateStruct.needsUpate)
		...
		if(updateStruct.needsUpate)
		{
			...
			if(sum == INTARRAYNONLEAFSIZE)
			{
				// the non-leaf node is full, needs to update upper level
				returnStruct.needsUpate = true;
				returnStruct.updateTuple = nonLeafNodeInsertSplit(node, updateStruct.updateTuple);

				// std::cout << "in insertEntryUpdate level = 0 feedbackTuple.pageNo = " << feedbackTuple.pageNo << std::endl;

				// test only yanqi
				assert(returnStruct.updateTuple.pageNo != 0);
			}
			else
			{
				// needsUpate = false;
	...
	if(isRoot)
	{
		...
		if(returnStruct.needsUpate)
		{
			...
			for(int i = 1; i < INTARRAYNONLEAFSIZE; ++i)
				rootNode->validArray[i] = false;
			rootNode->validArray[0] = true;
			rootNode->keyArray[0] = returnStruct.updateTuple.key;
			rootNode->pageNoArray[0] = rootPageNum;
			rootNode->pageNoArray[1] = returnStruct.updateTuple.pageNo;
