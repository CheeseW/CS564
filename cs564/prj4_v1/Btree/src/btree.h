 ...
 */
//                                                  sibling ptr             key               rid          valid bit
const  int INTARRAYLEAFSIZE = ( Page::SIZE - sizeof( PageId ) ) / ( sizeof( int ) + sizeof( RecordId ) + sizeof(bool));
// const int INTARRAYLEAFSIZE = 12;
/**
 * @brief Number of key slots in B+Tree non-leaf for INTEGER key.
 */
//                                                     level     extra pageNo                  key       pageNo             valid bit
const  int INTARRAYNONLEAFSIZE = ( Page::SIZE - sizeof( int ) - sizeof( PageId ) ) / ( sizeof( int ) + sizeof( PageId ) + sizeof(bool) );
// const int INTARRAYNONLEAFSIZE = 12;
/**
...
struct IndexMetaInfo{
  ...
  // added by yanqi CS564
  // indicate whether this B+ tree index has non-leaf nodes
  bool hasNonLeaf;
};
 ...
 * added by yanqi CS564
 * For each (key1, pid, key2), its value are within [key1, key2) 
 */
struct NonLeafNodeInt{
  ...
  // added by yanqi CS564
  // valid bits for keys (interval separator)
  // the ptrs right before and after a valid key are valid page number
  bool validArray[ INTARRAYNONLEAFSIZE ];
};
...
struct LeafNodeInt{
  ...
  // added by yanqi CS564
  bool validArray[ INTARRAYLEAFSIZE ];
...
class BTreeIndex {
 ...
 private:
  ...
  // added by yanqi CS564
  // indicate whether this B+ tree index has non-leaf nodes
  // TODO: update this value for b+ tree insertion
  bool hasNonLeaf;
   ...
   * @param node: ptr of the to be updated node
   ...
   * @param feedbackTuple: contains 1 pid + 1 key for upper level updates
   */
  void leafNodeInsertSplit(LeafNodeInt* curNode, int key, RecordId rid, PageKeyPair<int>& feedbackTuple);
   ...
   */
  void nonLeafNodeInsert(NonLeafNodeInt* node, const PageKeyPair<int>& updateTuple);

   /** 
   * function added by yanqi CS564
   * used to split & insert & sort & compact the data in a leaf node
   * caller is responsible for making sure that the leaf page is overflowed when adding a new (key, pid) tuple
   * @param curNode: ptr of the to be updated node
   * @param updateTuple: key and pid used to update the current node
   * @param feedbackTuple: the feedback information used to update the upper levels
   */
  void nonLeafNodeInsertSplit(NonLeafNodeInt* curNode, const PageKeyPair<int>& updateTuple, PageKeyPair<int>& feedbackTuple);

  /**
   * added by yanqi CS564
   * recursive function used to insert a tuple
   * @param key: key to be inserted, fixed as integer type
   * @param rid: the RecordId to be inserted into index
   * @param pid: the PageId of the page to be operated on
   * @param isRoot: if we at the root of the B+ tree
   * @param needsUpdate: feedback information from recurssion, set to true when the current level needs update
   * @param updateTuple: feedback info from recurssion. The key and 2 pids used to update the current level
   */
  void insertEntryUpdate(int key, RecordId rid, PageId pid, bool isRoot, bool& needsUpate, PageKeyPair<int>& feedbackTuple);


  /**
   * added by yanqi CS564
   * used when builing index on already existing index file to read all index file pages into buffer manager
   */
  // void readAllPages();
