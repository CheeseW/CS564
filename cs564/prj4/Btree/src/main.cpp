...
int main(int argc, char **argv)
{
  ...
  std::cout << "leaf size:" << badgerdb::INTARRAYLEAFSIZE << " non-leaf size:" << badgerdb::INTARRAYNONLEAFSIZE << std::endl;
...
void createRelationForward()
{
  ...
  for(int i = 0; i < relationSize; i++ )
	...
	// std::cout << "RelationshipForward succeeded" << std::endl;
}
...
void intTests()
{
  ...
  index.showLeavesKey();
  std::cout << "show done" << std::endl << std::endl;
  ...
  int numResults = 0;
	...
	while(1)
	{
		try
		{
			...
			// std::cout << "before unPinPage in intScan" << std::endl;
